...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (categoryRepo.count() == 0) {
            ...
            // Update ALL existing categories with correct images
        ...
        // Product image mapping (using hyphens instead of spaces for URL safety)
        ...
        // Update ALL existing products with slugs and correct images
        if (productRepository.count() > 0) {
            productRepository.findAll().forEach(p -> {
                ...
                // Always update image to correct filename
                String imageName = productImages.get(p.getTitle());
                if (imageName != null && !imageName.equals(p.getImage())) {
